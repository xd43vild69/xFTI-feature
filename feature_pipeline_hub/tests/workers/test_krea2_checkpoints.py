...
def populate(directory: Path, steps: list[int]) -> None:
    for step in steps:
        (directory / f"{PREFIX}_step_{step}.safetensors").write_text("x")
    (directory / f"{PREFIX}_FINAL.safetensors").write_text("x")
...
def test_ordering_is_numeric_not_lexicographic(tmp_path: Path) -> None:
    ...
    rotate(str(tmp_path), 1, PREFIX, log=lambda _m: None)
...
def test_zero_or_negative_keeps_everything(tmp_path: Path) -> None:
    for keep in (0, -1):
        ...
        rotate(str(directory), keep, PREFIX, log=lambda _m: None)
...
def test_keeping_more_than_exist_removes_nothing(tmp_path: Path) -> None:
    ...
    assert rotate(str(tmp_path), 10, PREFIX, log=lambda _m: None) == []


def test_the_final_export_and_unrelated_files_are_never_touched(tmp_path: Path) -> None:
    populate(tmp_path, [25, 50, 100])
    rotate(str(tmp_path), 1, PREFIX, log=lambda _m: None)
    assert (tmp_path / f"{PREFIX}_FINAL.safetensors").exists()
    assert (tmp_path / "train_log.csv").exists()


def test_a_different_prefix_in_the_same_directory_is_untouched(tmp_path: Path) -> None:
    """rotate() only prunes files matching its own prefix — relevant if the same
    output_dir ever ends up holding checkpoints from two differently-named runs."""
    populate(tmp_path, [25, 50, 100])
    (tmp_path / "Other_step_10.safetensors").write_text("x")
    rotate(str(tmp_path), 1, PREFIX, log=lambda _m: None)
    assert (tmp_path / "Other_step_10.safetensors").exists()


def test_returns_what_it_removed(tmp_path: Path) -> None:
    populate(tmp_path, [25, 50, 100])
    removed = rotate(str(tmp_path), 1, PREFIX, log=lambda _m: None)
    assert [Path(p).name for p in removed] == [
        f"{PREFIX}_step_25.safetensors", f"{PREFIX}_step_50.safetensors"]
